...
def _worker_mcts_run(species: str, targets: Iterable[str], root_state: State, n_simulations: int, max_rollout_depth: int, c: float = math.sqrt(2.0), seed: int | None = None) -> Dict[Action, Dict[str, float]]:
  """
...
def parallel_mcts_root(species: str, targets: Iterable[str], root_state: State, total_simulations: int = 1000, max_rollout_depth: int = 20, n_workers: int = 4, c: float = math.sqrt(2.0), seeds: List[int] | None = None) -> tuple[Action | None, Dict[Action, Dict[str, float]]]:
  """
  Root-parallel MCTS:
  ...
  with mp.Pool(processes=n_workers) as pool:
    results = pool.starmap(
        _worker_mcts_run,
        [(species, targets, root_state, sims_per_worker, max_rollout_depth, c, None if i is None else i) for i in seeds],
    )
